    ...
    class Character
    {
        ...
        public static void CharacterList(Packet packet)
        {
            ...
            catch (Exception ex)
            {
              // Globals.Debug("CharList", ex.Message, packet);
                  if (!BotData.use_client)
                        {
                Globals.UpdateLogs("Error On CharList, Please Insert Manual Char Name");
        ...
        public static void CharData(Packet packet)
        {
            try
            {
                ...
                Globals.MainWindow.slotbox.Items.Clear();
                ...
                packet.data.ReadDWORD(); // GatheredExpPoint (Use?)
                Character.CurrentHP = packet.data.ReadDWORD();
                Character.CurrentMP = packet.data.ReadDWORD();
                packet.data.ReadBYTE(); // AutoInverstExp(1 = Beginner Icon, 2 = Helpful, 3 = Beginner&Helpful)
                packet.data.ReadBYTE(); //DailyPK
                packet.data.ReadWORD(); //TotalPK
                packet.data.ReadDWORD(); // PKPenaltyPoint
                packet.data.ReadBYTE(); //nao sei
                packet.data.ReadBYTE(); //HwanLevel
                packet.data.ReadBYTE(); //unk01 -> Check for != 0
                #endregion
                #region Items
                ...
                for (int y = 0; y < Character.itemscount; y++)
                {
                    ...
                    Globals.MainWindow.slotbox.Items.Add(slot);
                    ...
                    if (type.StartsWith("ITEM_CH") || type.StartsWith("ITEM_EU") || type.StartsWith("ITEM_MALL_AVATAR") || type.StartsWith("ITEM_EVENT_AVATAR") || type.StartsWith("ITEM_ETC_E060529_GOLDDRAGONFLAG") || type.StartsWith("ITEM_EVENT_CH") || type.StartsWith("ITEM_EVENT_EU") || type.StartsWith("ITEM_EVENT_AVATAR_W_NASRUN") || type.StartsWith("ITEM_EVENT_AVATAR_M_NASRUN"))
                    {
                        ...
                        packet.data.ReadQWORD(); //White stat
                    ...
                    else if (type.Contains("ITEM_ETC_ARCHEMY_ATTRSTONE") || type.Contains("ITEM_ETC_ARCHEMY_LEVEL_ATTRSTONE") || type.Contains("ITEM_ETC_ARCHEMY_MAGICSTONE") || type.Contains("ITEM_ETC_ARCHEMY_LEVEL_MAGICSTONE"))
                    {
                        if (type.Contains("ITEM_ETC_ARCHEMY_MAGICSTONE_LUCK") || type.Contains("ITEM_ETC_ARCHEMY_LEVEL_MAGICSTONE_LUCK") || type.Contains("ITEM_ETC_ARCHEMY_LEVEL_MAGICSTONE_SOLID") || type.Contains("ITEM_ETC_ARCHEMY_MAGICSTONE_SOLID"))
                        {
                            ...
                            Char_Data.inventorycount.Add(count1);
                            Char_Data.inventorydurability.Add(0);
                        }
                        else
                        {
                            ...
                            packet.data.ReadBYTE(); // nao sei
                            Char_Data.inventorycount.Add(count2);
                            Char_Data.inventorydurability.Add(0);
                        }
                    }
                    else
                ...
                Globals.MainWindow.Text = Globals.botTitle + " | " + Character.PlayerName + " | " + Globals.MainWindow.in_game_server_name.Text;
                Globals.MainWindow.tray.Text = Globals.botTitle + " | " + Character.PlayerName + " | " + Globals.MainWindow.in_game_server_name.Text;
                if (data_loaded == 0)
                {
            ...
            try
            {
                ...
                switch (code)
                {
                    case 1:
                        #region Gold Update
                        Character.Gold = packet.data.ReadQWORD();
                        Globals.MainWindow.lb_gold1.Text = Character.Gold.ToString();
                        break;
                        #endregion
                    case 2:
                        #region SP Update
                    ...
                    case 4:
                        #region Zerk Update
                        Character.Zerk = packet.data.ReadBYTE();
                        Globals.MainWindow.zerk_bar1.Value = (int)Character.Zerk;
        ...
        public static void CharacterInfo(Packet packet)
        {
            try
            {
                ...
                Globals.MainWindow.lb_level1.Text = Character.Level.ToString();
                Globals.MainWindow.lb_charname1.Text = Character.PlayerName;
                Globals.MainWindow.lb_sp1.Text = Character.SkillPoints.ToString();
                ...
                Globals.MainWindow.label_race.Text = Character.Race;
                ...
                Globals.MainWindow.mp_bar1.Maximum = (int)Character.MaxMP;
                Globals.MainWindow.mp_bar1.Value = (int)Character.CurrentMP;
                Globals.MainWindow.MPShow1.Text = (int)Character.CurrentMP + "/" + (int)Character.MaxMP;
                Globals.MainWindow.hp_bar1.Maximum = (int)Character.MaxHP;
                Globals.MainWindow.hp_bar1.Value = (int)Character.CurrentHP;
                Globals.MainWindow.hpshow1.Text = (int)Character.CurrentHP + "/" + (int)Character.MaxHP;
               // if (Character.expmax > Int64.MaxValue)
                //{
                  //  Globals.MainWindow.exp_bar1.Maximum = 100;
                    //if (Character.expmax > 0)
                      //  Globals.MainWindow.exp_bar1.Value = (int)((Character.exp / Character.expmax) * 100);
                //}
                //else
                //{
                  //  Globals.MainWindow.exp_bar1.Maximum = (int)Character.expmax;
                  //Globals.MainWindow.exp_bar1.Value = (int)Character.exp;
                //}
                int goodvenus = 100;
                int getliik = (int)((Character.exp / Character.expmax) * 100);
                double prozentXP;
                if ((int)Character.expmax > 0)
                {
                    prozentXP = Math.Round(((double)(getliik - 0) / (double)(goodvenus - 0)) * 100, 2);
                }
                else
                {
                    prozentXP = Math.Round(((double)(getliik - 0) / (double)(goodvenus - 0 + 100)) * 100, 2);
                }
                Globals.MainWindow.xpPercentLabel1.Text = prozentXP.ToString() + "%";
                //Globals.MainWindow.exp_bar.Maximum = (int)Character.expmax;
                //Globals.MainWindow.exp_bar.Value = (int)Character.exp;
                Globals.MainWindow.zerk_bar1.Value = (int)Character.Zerk;
                Globals.MainWindow.zerkn1.Text = (int)Character.Zerk + "";
                BotData.returning = 0;
                BotData.dead = false;
            }
            catch (Exception ex)
        ...
        public static void GuildAnalyze(Packet packet)
        {
            try
            {
                ...
                Globals.MainWindow.lb_guildname1.Text = Character.guildName;
            }
            catch (Exception ex)
            {
